...
global path_num
path_num=1
global local_control_cmd
local_control_cmd = [0,0,0,0]
...
dynamic_deadline = 0.09

HEADER = 64
PORT = 5064
FORMAT = 'utf-8'
DISCONNECT_MESSAGE = "!DISCONNECT"
SERVER = "127.0.0.1" #"18.144.167.144"
ADDR = (SERVER, PORT)
...
def plot_results(path, x_history, y_history, timeout):
    ...
    plt.figure()
    plt.title("PID Tracking Results - Dynamic Local")
    ...
    plt.legend()
    # plt.show()
    plt.savefig(f"Dynmaic_Local_PathNum_{path_num}_high_traffic.png")
...
def recv(client):
    ...
    global last_cmd_time, local_control_cmd
    ...
    # to_actuate(prev_cmd_time)
    local_control_cmd = [state[0], state[1], state[2], state[3]]
...
def reset_sim(path_var, timeout):
    ...
    global sensing_tick, actuation_tick, last_cmd_time, dynamic_deadline, local_control_cmd
...
def start_sim(path_var, timeout):
    ...
    global x_history, y_history, local_control_cmd, dynamic_deadline
    ...
    while True:
        ...
        if curr_state == state_txd and timer_expired(last_state_time, timeout):
            ...
            set_ctrl(car, local_control_cmd[0], local_control_cmd[1], local_control_cmd[2], local_control_cmd[3])
            curr_state = state_rxvd
