...
def rrt_path(obstacles, xy_start, xy_goal, params):
    ...
    while not nearGoal: # and iters < maxiters:
        ...
        collFree = t.isCollisionFreeVertex(obstacles, xy)
        ...
        collFree = t.isCollisionFreeEdge(obstacles, closest_node.p, new_node.p)
        # If it's not collision free, continue with loop
        if not collFree:
            iters += 1
            continue
        ...
        if norm(np.array(xy_goal) - np.array(new_node.p)) < minDistGoal:
            ...
            if t.isCollisionFreeEdge(obstacles, new_node.p, goal_node.p):
